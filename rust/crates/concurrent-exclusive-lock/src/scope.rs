use crate::{
    ...
    ExclusiveGuard, CONVERGE_ADD, EXCLUSIVE_ADD, MAX_CONCURRENT,
};
...
pub struct ConcurrentExclusiveLockScope<'a> {
    ...
    counter_mate: i64,
    exclusive_guard: Option<ExclusiveGuard<'a>>,
...
impl<'a> ConcurrentExclusiveLockScope<'a> {
    ...
    pub fn try_acquire_concurrent(&mut self) -> Result<Option<i32>, ConcurrentExclusiveLockError> {
    ...
    pub fn acquire_exclusive(&mut self) {
        self.exclusive_guard = Some(self.lock.acquire_exclusive());
    ...
    pub fn try_acquire_exclusive(&mut self, preempt_concurrent: bool) -> bool {
        if let Some(guard) = self.lock.try_acquire_exclusive(preempt_concurrent) {
    ...
    pub fn try_acquire_exclusive_for(&mut self, timeout: Duration) -> bool {
        if let Some(guard) = self.lock.try_acquire_exclusive_for(timeout) {
            self.exclusive_guard = Some(guard);
            self.counter_mate += EXCLUSIVE_ADD;
            true
        } else {
    ...
    pub fn release_exclusive(&mut self) {
        let guard = self
            .exclusive_guard
            .take()
            .expect("scope does not hold Exclusive permission");
    ...
    pub fn exclusive_to_concurrent(&mut self) {
        let guard = self
            ...
            .take()
            .expect("scope does not hold Exclusive permission");
        self.lock.exclusive_to_concurrent(guard);
    ...
    pub fn concurrent_to_exclusive(&mut self) {
        self.exclusive_guard = Some(self.lock.concurrent_to_exclusive());
        self.counter_mate += CONVERGE_ADD;
    }
    ...
    /// On failure, the original Concurrent permission is already released and
    /// the scope records no permission for that access chain.
    pub fn try_concurrent_to_exclusive_with_raise_epoch_id(&mut self, new_epoch_id: i32) -> bool {
        if let Some(guard) = self
            .lock
    ...
    pub fn release_all(&mut self) {
        if let Some(guard) = self.exclusive_guard.take() {
            drop(guard);
            self.counter_mate -= EXCLUSIVE_ADD;
        }
        if self.counter_mate != 0 {
            self.lock.free_release_concurrent(self.counter_mate);
            self.counter_mate = 0;
        }
    }
}
