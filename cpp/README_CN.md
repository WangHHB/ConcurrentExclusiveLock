...
下面列出两组相互独立的 Release 构建内存负载测试。它们用于提供可复现的实测参考，不代表跨平台的普遍性能结论。Linux 与 Windows 使用了不同硬件和不同 Work 参数，因此不能直接比较两台机器的绝对吞吐值。
...
CEL 与 `std::shared_mutex` 的对比：

| 读/写比例 | `std::shared_mutex` 吞吐量 | CEL 吞吐量 | CEL 吞吐差异 | `std::shared_mutex` 平均写入时间 | CEL 平均写入时间 | 写入延迟对比 |
|---:|---:|---:|---:|---:|---:|---:|
| 100/0 | 4,902,155 works/s | 4,771,451 works/s | -2.67% | — | — | — |
| 99.5/0.5 | 3,282,886 works/s | 3,421,835 works/s | +4.23% | 497.30 μs | 89.85 μs | 降低 5.54 倍 |
| 90/10 | 1,629,067 works/s | 3,108,759 works/s | +90.83% | 59.95 μs | 7.97 μs | 降低 7.52 倍 |
| 50/50 | 2,663,306 works/s | 2,687,139 works/s | +0.89% | 8.14 μs | 5.11 μs | 降低 1.59 倍 |
| 30/70 | 2,872,152 works/s | 2,734,470 works/s | -4.79% | 6.16 μs | 4.82 μs | 降低 1.28 倍 |
| 0/100 | 2,334,148 works/s | 2,701,963 works/s | +15.76% | 5.71 μs | 5.06 μs | 降低 1.13 倍 |

### Windows：单锁 64 线程高竞争

```text
编译器/运行环境： MSVC Release，Windows
报告 CPU：        16
锁实例数：        1
每把锁线程数：    64
总线程数：        64
业务负载：        memory，单锁共享 64 MiB
每线程操作数：    10,000
每策略每场景总量：640,000 次锁操作
读写工作量：      64 / 128 steps
```
...
| 读/写比例 | `std::shared_mutex` 吞吐量 | CEL 吞吐量 | CEL 吞吐差异 | `std::shared_mutex` 平均写入时间 | CEL 平均写入时间 | 写入延迟对比 |
|---:|---:|---:|---:|---:|---:|---:|
| 100/0 | 6,867,130 works/s | 7,138,491 works/s | +3.95% | — | — | — |
| 99.5/0.5 | 3,608,730 works/s | 3,552,412 works/s | -1.56% | 253.19 μs | 30.28 μs | 降低 8.36 倍 |
| 90/10 | 916,086 works/s | 961,376 works/s | +4.94% | 179.91 μs | 75.39 μs | 降低 2.39 倍 |
| 50/50 | 334,891 works/s | 375,224 works/s | +12.04% | 175.04 μs | 173.40 μs | 基本相同 |
| 30/70 | 270,018 works/s | 299,836 works/s | +11.04% | 182.01 μs | 214.06 μs | 增加 17.61% |
| 0/100 | 235,043 works/s | 200,049 works/s | -14.89% | 270.48 μs | 316.45 μs | 增加 17.00% |

### 客观解读

这些结果没有显示出一个在所有场景中都占优的吞吐赢家，而是呈现出更有价值、也更容易复现的规律：

- 纯 Concurrent 场景中，CEL 在两个环境里都与 `std::shared_mutex` 非常接近：Linux 为 -2.67%，Windows 为 +3.95%。
