...
All language versions have undergone performance benchmarking and long-running stress testing. The C# reference implementation has additionally completed a unified evaluation matrix covering a single-core configuration, SMT enabled and disabled, a 4-vCPU virtual machine, and dual-socket 52-core / 104-thread systems running both Windows and Linux. In workloads with meaningful Concurrent parallelism, a need for timely Exclusive progress, or frequent permission convergence, CEL significantly outperformed ReaderWriterLockSlim across most evaluated configurations and could also outperform mutex-based serialization when sufficient compatible work was available. In write-heavy, purely Exclusive, or parallelism-limited workloads, its performance generally remained close to mutex-level behavior, with no systematic degradation attributable to the protocol structure.**
...
See the [C# Testing and Benchmark Guide](./csharp/TestAndBenchmark/README.md).

<a href="CEL_Intel Platinum 8269CY @ 2.50GHz, 2 Sockets 52 Cores 104 Threads, Ubuntu 26.04.md">Dual-Socket Intel Xeon Platinum 8269CY (52 Cores, 104 Threads, Ubuntu 26.04) Complete Benchmark Report</a>
